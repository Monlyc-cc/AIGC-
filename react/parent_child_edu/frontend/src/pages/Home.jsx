...
export default function Home() {
  const quickEntries = [
    ...
    { title: '睡前故事馆', desc: 'AI 讲故事，个性化选择角色与情节', tag: '故事',path:'sleep-stores' },
        ...
        <div className="home-grid">
          {
            quickEntries.map((item,index)=>{
             return  <HomeCard key={index}  path={item.path} tag={item.tag} desc={item.desc} title={item.title}></HomeCard>
            })
          }
          <div className="home-card">
            <div className="home-card__tag">AI识别</div>
            <h3>拍照识物</h3>
            <p>识别身边的物品</p>
            <button className='home-btn home-btn--small'>进入</button>
          </div>
        </div>
      </section>
    </div>
